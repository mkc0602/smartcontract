...
contract MultiVACPad is IERC20, Ownable, BaseToken {
    ...
    uint256 public _distributionFee;
    ...
    constructor(
        ...
        uint16 distributionFeeBps_,
        uint16 liquidityFeeBps_,
        uint16 teamFeeBps_
    ) payable {
        require(distributionFeeBps_ >= 0, "Invalid tax fee");
        ...
        require(
            distributionFeeBps_ + liquidityFeeBps_ + teamFeeBps_ <= 10**4 / 4,
        ...
        _distributionFee = distributionFeeBps_;
    ...
    function setDistributionFeePercent(uint256 distributionFeeBps) external onlyOwner {
        _distributionFee = distributionFeeBps;
        require(
            _distributionFee + _liquidityFee + _teamFee <= 10**4 / 4,
            "Total fee is over 25%"
        );
    }
    ...
    function _takeTeamFee(uint256 tTeam) private {
        ...
        // swap tokens for ETH
        swapTokensForEth(tTeam);

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        //only 2% of team fee is sent.Other balance not touched
    ...
    function calculateDistributionFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_distributionFee).div(10**4);
    }
    ...
    function removeAllFee() private {
        if (_distributionFee == 0 && _liquidityFee == 0 && _teamFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        function initiateTeamFeeTransfer() external onlyOwner() {
